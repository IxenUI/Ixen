...
then
    ...
    then
        ...
        read -r device_w device_h scale < "$said"

        if dotnet run --project "$tools" -c "$configuration" --no-build -- --render "$library" "$device_w" "$device_h" "$rendered" --component MainComponent --scale "$scale" > "$work/render.txt" 2>&1 && [ -s "$rendered" ]
        then
            ...
            if [ "$one" = "$two" ]
            then
                Record OK "host frame matches library" "$device_w x $device_h at scale $scale, $one, $moved"
            else
                dotnet run --project "$tools" -c "$configuration" --no-build -- --diff "$shot" "$rendered" 2>&1 | grep '^difference' | sed 's/^/     /'
                Record FAIL "host frame matches library" "the X11 path and the library disagree at $device_w x $device_h scale $scale ($moved)"
            fi
        else
