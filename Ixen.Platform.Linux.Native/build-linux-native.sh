...
if ! pkg-config --exists x11 xcursor dbus-1; then
...
SOURCES="window/native_window.c accessibility/atspi.c api/window_api.c"
FLAGS="-std=gnu11 -fPIC -fvisibility=hidden -Wall -Wextra $OPT -shared"
LIBS="$(pkg-config --cflags --libs x11 xcursor dbus-1)"
